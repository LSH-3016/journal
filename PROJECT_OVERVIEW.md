...
### Kubernetes 배포 파일
```
├── k8s/                        # Kubernetes 매니페스트 (ArgoCD가 읽는 폴더)
│   ├── k8s-deployment.yaml    # Deployment, Service, ServiceAccount 정의
│   └── k8s-ingress.yaml       # ALB Ingress 설정
└── argocd-application.yaml    # ArgoCD Application 정의 (path: k8s)
```
...
- **ServiceAccount**: IRSA를 통한 AWS 권한 부여 (journal-api-secrets-role)
- **환경변수**: AWS_REGION, ALLOWED_ORIGINS, ENVIRONMENT=production, S3_BUCKET_NAME, BEDROCK_MODEL_ID
...
- SSL 인증서: `arn:aws:acm:us-east-1:324547056370:certificate/dcba4e4a-c0d5-4e97-aecc-91a1b35f7355`
...
- k8s/k8s-deployment.yaml 이미지 태그 업데이트
- Git 커밋 및 푸시 (ArgoCD가 감지)
- GITHUB_TOKEN 사용 (permissions: contents: write)
...
- `ENVIRONMENT=production` 시 Secrets Manager에서 자동 로드
- `ENVIRONMENT=development` 시 환경변수 사용
- Secret 이름: `journal-api/database`
- 필수 키: host, port, dbname, username, password
